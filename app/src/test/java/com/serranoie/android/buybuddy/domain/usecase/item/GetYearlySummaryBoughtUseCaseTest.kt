...
class GetYearlySummaryBoughtUseCaseTest {

    private lateinit var repository: ItemRepositoryImpl
    private lateinit var useCase: GetYearlySummaryBoughtUseCase

    @Before
    fun setup() {
        repository = mockk()
        useCase = GetYearlySummaryBoughtUseCase(repository)
    }
    ...
    fun `invoke emits Success when repository returns data`() = runBlocking {
        ...
        coEvery { repository.getMonthlySumForItemsWithStatusOne() } returns flowOf(data)
    ...
    fun `invoke emits Error when repository throws exception`() = runBlocking {
        ...
        coEvery { repository.getMonthlySumForItemsWithStatusOne() } throws exception
